...
range(d$Game1Angry1) #interesting, returns NA. Guess it's because range will return if any obs are missing 
is.na(d$Game1Angry1)
...
NOTE: there's also a duplicate of participant 23's observations. 
...
It's good practice to assign a new variable name (in this case `filtered_d`) to a data frame when you change it in an important way, or apply a code chunk that shouldn't be run twice. This helps prevent you seeing different results when syou run your code in chunks (and might run one multiple times, or skip it, etc.) vs. knit the document.
