...
author: "Andrew Lampinen (Pset2 Completed by Cid Decatur) "
...
performance_hyp_d = filtered_d %>% 
  select(Subject,
         ...
         starts_with("SOF"),
         -contains("Game"),
         -DoNotUseVideoGamePerformanceData) # your code here: remove the columns containing "Game" in the name
```
...
```{r, echo=TRUE}
model = lm(Score ~ ConfrontationalGame * Emotion, rating_hyp_long_d)
summary(model)
```
...
performance_diff_summary_d = performance_diff_d %>%
  ...
  summarize(MeanImprovementScore = mean(ImprovementScore, na.rm = TRUE)) %>%
  ungroup()
```
